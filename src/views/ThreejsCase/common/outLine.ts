...
import chinaOutLineJson from '@/assets/json/china-outline.json'
import { lglt2xyz } from './helper'
    ...
    coordinates.forEach(multiPolygon => {
      multiPolygon.forEach(polygon => {
        if (polygon.length > 200) {
          ...
          let color = new Vector3(0.5999758518718452, 0.7798940272761521, 0.6181903838257632)
...
export const initLineMaterial = (setting: any) => {
  ...
  const color = setting ? setting.color || new Vector3(0, 1, 1) : new Vector3(0, 1, 1)
