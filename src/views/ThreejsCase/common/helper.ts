...
const _3Dto2D = (start: Vector3, end: Vector3) => {
  ...
  const xoy_quaternion = new Quaternion().setFromUnitVectors(normal, new Vector3(0, 0, 1))
  const start_xoy = start.clone().applyQuaternion(xoy_quaternion)
  const end_xoy = end.clone().applyQuaternion(xoy_quaternion)

  const middle_xoy = new Vector3().addVectors(start_xoy, end_xoy).multiplyScalar(0.5).normalize()
  const xoy_quaternion_middle = new Quaternion().setFromUnitVectors(middle_xoy, new Vector3(0, 1, 0))
  const start_xoy_middle = start_xoy.clone().applyQuaternion(xoy_quaternion_middle)
  ...
  const quaternionInverse = xoy_quaternion.clone().invert().multiply(xoy_quaternion_middle.clone().invert())
  return {
    // 返回两次旋转四元数的逆四元数
...
function getFunctionExpression(src: Vector3, dist: Vector3) {
  ...
  return getPointByDistance(x1, y1, k_perpendicular, b_perpendicular, distance / 5)
}
//根据已知函数距离求点位
function getPointByDistance(x1: number, y1: number, k: number, b: number, s: number) {
  // 求解一元二次方程
...
export { lon2xyz, _3Dto2D, radianAOB, threePointCenter, getFunctionExpression, getPointByDistance }
